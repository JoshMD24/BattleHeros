...
FVector AAStar::FindPath(FVector Start, FVector End)
{
	AAStarNode* StartNode = GridInstance->NodeFromLocation(Start);	// Start node of the agent
	AAStarNode* EndNode = GridInstance->NodeFromLocation(End);	// End goal node of the agent



	if (StartNode == nullptr) return Start;

	TArray<AAStarNode*> OpenSet; // Nodes to be checked
	TArray<AAStarNode*> ClosedSet; // Nodes already checked
	...
	while (OpenSet.Num()) // While there is still nodes to be checked keep running the algorithm
	{
		...
		AAStarNode* CurrentNode = OpenSet[0]; // Run each cycle from the beginning element of the open list as the previous first element will have been processed and place into the closed list
		...
		TArray<AAStarNode*> Neighbours = GridInstance->GetNeighbourNodes(CurrentNode); // Gather CurrentNodes neighbours into the list
...
float AAStar::GetDistance(AAStarNode* A, AAStarNode* B)
{
...
FVector AAStar::RetracePath(AAStarNode* StartNode, AAStarNode* EndNode)
{
	...
	TArray<AAStarNode*> Path;

	AAStarNode* CurrentNode = EndNode;
