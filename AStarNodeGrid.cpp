...
// Called every frame
void AAStarNodeGrid::Tick(float DeltaTime)
{
...
void AAStarNodeGrid::BuildGrid()
{
	for (int x = 0; x < GridSize.X; x++)
	{
		NodeGrid.Add(TArray<TArray<AAStarNode*>>());
		for (int y = 0; y < GridSize.Y; y++)
		{
			NodeGrid[x].Add(TArray<AAStarNode*>());
			for (int z = 0; z < GridSize.Z; z++)
			{
				...
				AAStarNode* currentNode = new AAStarNode(x, y, z);
				if (hit)
				{
...
TArray<AAStarNode*> AAStarNodeGrid::GetNeighbourNodes(AAStarNode* const& Node)
{
...
AAStarNode* AAStarNodeGrid::NodeFromLocation(FVector const& loc)
{
	...
	AAStarNode* test = NodeGrid[RoundedGridLocation.X][RoundedGridLocation.Y][RoundedGridLocation.Z];

	return NodeGrid[RoundedGridLocation.X][RoundedGridLocation.Y][RoundedGridLocation.Z];
}

FVector AAStarNodeGrid::LocationFromNode(AAStarNode* const& Node)
{
